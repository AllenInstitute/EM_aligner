...
url = sprintf('%s/owner/%s/project/%s/stack/%s/z/%s/world-to-local-coordinates/%.2f,%.2f',...
...
url = sprintf('%s/owner/%s/project/%s/stack/%s/tile/%s/local-to-world-coordinates/%.2f,%.2f',...
