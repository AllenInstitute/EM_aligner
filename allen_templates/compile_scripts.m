...
cmd1 = sprintf('mcc -m -R -nodesktop -v %s/matlab_compiled/system_solve_affine_with_constraint_SL.m -N -p %s -a %s -a %s -d %s', EM_ALIGNER_ROOT, EM_ALIGNER_ROOT, solver_scripts, jsonlab_scripts, OUTPUT_FOLDER);
cmd2 = sprintf('mcc -m -R -nodesktop -v %s/matlab_compiled/system_solve_rigid_approximation_SL.m -N -p %s -a %s -a %s -d %s', EM_ALIGNER_ROOT, EM_ALIGNER_ROOT, solver_scripts, jsonlab_scripts, OUTPUT_FOLDER);
cmd3 = sprintf('mcc -m -R -nodesktop -v %s/matlab_compiled/system_solve_rigid_approximation_SL.m -N -p %s -a %s -a %s -d %s', EM_ALIGNER_ROOT, EM_ALIGNER_ROOT, solver_scripts, jsonlab_scripts, OUTPUT_FOLDER);
