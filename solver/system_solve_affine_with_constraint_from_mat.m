function [err,R, Tout, Diagnostics] = system_solve_affine_with_constraint(matfilename,newstackname)
%
% steps in right after forming K from a mat file. names the stack something
% new. useful for stepping into a saved solution and running through the
% command line for profiling. i.e. with cgmemtime. See breakpoints folder.
%
...
% %     timer_solve_A = tic;
% 
% disp('breakpoint for saving K and Lm');

load(matfilename);
rcout.stack = newstackname;
...
tmp = strsplit(matfilename,'.mat');
dname = strcat(tmp{1},'_',newstackname,'_diagnostics');
save(dname,'Diagnostics');
